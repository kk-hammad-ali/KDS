...
@section('content')
    ...
    <div class="container mt-4">
        <div class="border rounded p-3" style="border-color: green;">
            <div class="d-flex justify-content-between align-items-center">
                <img id="question-image" src="" alt="Sign" class="img-fluid"
                    style="display: none; height:50px; width:50px;">
                <h5 class="text-primary">آن لائن سگنل ٹیسٹ (<span id="current-question">1</span> میں سے
                    {{ count($urduQuestions) }})</h5>
            </div>
            <hr>
            <h4 class="text-right" id="question-text">{{ $urduQuestions[0]['question'] }}</h4>
    ...
    <script>
        const questions = @json($urduQuestions); // Import questions from PHP
        ...
        function loadQuestion() {
            ...
            question.options.forEach(option => {
                ...
                label.classList.add("list-group-item", "d-flex", "justify-content-end", "align-items-center",
                    "signal-option");
                label.innerHTML = `
            <input type="radio" name="signal" class="form-check-input ms-5" value="${option.id}">
            <h5 class="me-5"  style="margin-right: 50px;" >${option.text || ''}</h5>
            ${option.image ? `<img src="${option.image}" alt="Option Image" class="img-thumbnail ms-3" style="margin-right: 50px; width: 100px; height: 100px;">` : ''}
        `;
        ...
        document.getElementById("next-btn").addEventListener("click", () => {
            if (currentQuestionIndex < questions.length - 1) {
                ...
                // Display final result and replace the content with score in Urdu
                const resultDiv = document.createElement('div');
                resultDiv.innerHTML = `
            <div class="alert alert-info text-right">
                <h4>ٹیسٹ مکمل ہو چکا ہے!</h4>
                <p>کل سوالات: ${questions.length} <br> آپ کا اسکور: ${score}</p>
            </div>
        `;
